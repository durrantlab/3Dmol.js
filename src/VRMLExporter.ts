import { Object3D, Geometry, GeometryGroup, LineBasicMaterial, Material } from "./WebGL";
...
export class VRMLExporter {
    ...
    private parse(object: Object3D, options: VRMLExportOptions, indent: string): string {
        ...
        output += transformPropertyIndent + 'translation ' + object.position.x + ' ' + object.position.y + ' ' + object.position.z + '\n';
        ...
        output += transformPropertyIndent + 'rotation ' + axis.x + ' ' + axis.y + ' ' + axis.z + ' ' + angle + '\n';
        output += transformPropertyIndent + 'scale ' + object.scale.x + ' ' + object.scale.y + ' ' + object.scale.z + '\n';
    ...
    private parseGeometryGroup(geoGroup: GeometryGroup, material: Material, options: VRMLExportOptions, indent: string): string {
        ...
        if (material.color) {
            output += materialIndent + 'diffuseColor ' + material.color.r + ' ' + material.color.g + ' ' + material.color.b + '\n';
        }
        if (material.wireframe && geoGroup.colorArray) {
            const c = geoGroup.colorArray;
            output += materialIndent + "emissiveColor " + c[0] + " " + c[1] + " " + c[2] + "\n";
        }
        ...
        if (material instanceof LineBasicMaterial || material.wireframe) {
            output += this.generateIndexedLineSet(geoGroup, material, options, shapeIndent);
        } else {
            output += this.generateIndexedFaceSet(geoGroup, options, shapeIndent);
        }
        output += indent + '}\n';
    ...
    private formatCoord(val: number, precision?: number): string {
    ...
    private generateIndexedLineSet(geoGroup: GeometryGroup, material: Material, options: VRMLExportOptions, indent: string): string {
        ...
        for (let i = 0; i < geoGroup.vertices; ++i) {
            const offset = i * 3;
            const x = this.formatCoord(geoGroup.vertexArray?.[offset], options.precision);
            const y = this.formatCoord(geoGroup.vertexArray?.[offset + 1], options.precision);
            const z = this.formatCoord(geoGroup.vertexArray?.[offset + 2], options.precision);
    ...
    private generateIndexedFaceSet(geoGroup: GeometryGroup, options: VRMLExportOptions, indent: string): string {
        const mergeDist = options.mergeVertices;
        if (typeof mergeDist === 'number' && mergeDist > 0) {
            return this._generateMergedIndexedFaceSet(geoGroup, options, indent);
        }
        return this._generateUnmergedIndexedFaceSet(geoGroup, options, indent);
        }

    private _generateUnmergedIndexedFaceSet(geoGroup: GeometryGroup, options: VRMLExportOptions, indent: string): string {
        ...
        for (let i = 0; i < geoGroup.vertices; ++i) {
            const offset = i * 3;
            const x = this.formatCoord(geoGroup.normalArray?.[offset], options.precision);
            const y = this.formatCoord(geoGroup.normalArray?.[offset + 1], options.precision);
            const z = this.formatCoord(geoGroup.normalArray?.[offset + 2], options.precision);
            output += geoIndent + '  ' + x + ' ' + y + ' ' + z + ',\n';
        }
        output += geoIndent + ' ]\n' + geoIndent + '}\n';
        ...
        if (geoGroup.colorArray) {
            ...
            for (let i = 0; i < geoGroup.vertices; ++i) {
                const offset = i * 3;
                output += geoIndent + '  ' + geoGroup.colorArray[offset] + ' ' + geoGroup.colorArray[offset + 1] + ' ' + geoGroup.colorArray[offset + 2] + ',\n';
            }
            output += geoIndent + ' ]\n' + geoIndent + '}\n';
        }
    ...
    private _generateMergedIndexedFaceSet(geoGroup: GeometryGroup, options: VRMLExportOptions, indent: string): string {
        ...
        for (let i = 0; i < geoGroup.vertices; i++) {
                            ...
                            for (const newIndex of cell) {
                                ...
                                if (distSq < mergeDistSq) {
                                    ...
                                    if (geoGroup.normalArray) {
                                        newNormals[newIndex].x += geoGroup.normalArray[offset];
                                        newNormals[newIndex].y += geoGroup.normalArray[offset + 1];
                                        newNormals[newIndex].z += geoGroup.normalArray[offset + 2];
            ...
            if (!found) {
                ...
                if (geoGroup.normalArray) {
                    newNormals.push({ x: geoGroup.normalArray[offset], y: geoGroup.normalArray[offset + 1], z: geoGroup.normalArray[offset + 2], count: 1 });
                }
                if (geoGroup.colorArray) {
        ...
        for (const v of newVertices) {
            output += geoIndent + '  ' + this.formatCoord(v.x, options.precision) + ' ' + this.formatCoord(v.y, options.precision) + ' ' + this.formatCoord(v.z, options.precision) + ',\n';
        }
        output += geoIndent + ' ]\n' + geoIndent + '}\n';
        ...
        if (newNormals.length > 0) {
            output += geoIndent + 'normal Normal {\n' + geoIndent + ' vector [\n';
            for (const n of newNormals) {
                output += geoIndent + '  ' + this.formatCoord(n.x, options.precision) + ' ' + this.formatCoord(n.y, options.precision) + ' ' + this.formatCoord(n.z, options.precision) + ',\n';
            }
            output += geoIndent + ' ]\n' + geoIndent + '}\n';
        }
        ...
        for (let i = 0; i < newFaceArray.length; i += 3) {
            output += geoIndent + ' ' + newFaceArray[i] + ', ' + newFaceArray[i + 1] + ', ' + newFaceArray[i + 2] + ', -1,\n';
        }
        output += geoIndent + ']\n';
        output += indent + '}\n';
