...
export namespace GLDraw {
    ...
    class CylVertexCache {
        ...
        cache: any = {};
        // memoize basis vectors
        basisVectorsCache: any = {};
        getBasisVectors(subdivisions: number) {
            if (this.basisVectorsCache[subdivisions])
                return this.basisVectorsCache[subdivisions];
            let nvecs = [];
            ...
            for (i = 3; i <= subdivisions; i++) {
                ...
                spacing = N / M;
                for (j = 0; j < M - 1; j++) {
                    nvecs[spacing / 2 + j * spacing] = nvecs[j * spacing]
                        .clone()
                        .add(nvecs[(j + 1) * spacing])
                        .normalize();
                }
            ...
            this.basisVectorsCache[subdivisions] = nvecs;
            return nvecs;
        }
        constructor() {}
        getVerticesForRadius(
            radius: any,
            cap: CAP,
            capType: any,
            options: {
            ...
            const subdivisions = options.cylinderSubdivisions || 4;
            const heightSegments = options.cylinderHeightSegments || 10;
            const cacheKey = `${cap}${capType}_${subdivisions}_${heightSegments}`;
            if (
                typeof this.cache !== "undefined" &&
                this.cache[radius] !== undefined
            )
                if (this.cache[radius][cacheKey] !== undefined)
                    return this.cache[radius][cacheKey];

            const basisVectors = this.getBasisVectors(subdivisions);
            ...
            var widthSegments = w; // however many basis vectors for cylinder
            ...
            for (y = 0; y <= heightSegments; y++) {
                ...
                for (x = 0; x <= widthSegments; x++) {
                    ...
                    else if (polar) verticesRow.push(nvecs.length - 1);
                }

                // extra equator row
                if (equator) verticesRows.push(toRow);
            ...
            this.cache[radius][cacheKey] = obj;
            return obj;
        }
    }
     ...
     * @param {Object} [cylinderOptions] - Cylinder quality options
     *
     * */
    export function drawCylinder(
        ...
        // TODO: Will have to orient with model view matrix according to direction
        var vobj = cylVertexCache.getVerticesForRadius(
            radius,
            toCap as CAP,
            "to",
            cylinderOptions
        ...
        if (drawcaps) {
            ...
            for (y = ystart; y < yend; y++) {
                ...
                var toObj = cylVertexCache.getVerticesForRadius(
                    radius,
                    toCap as CAP,
                    "to",
                    cylinderOptions
                );
                var fromObj = cylVertexCache.getVerticesForRadius(
                    radius,
                    fromCap as CAP,
                    "from",
                    cylinderOptions
    ...
    export function drawCone(
        ...
        var basis = cylVertexCache.getBasisVectors(4);
        var n = basis.length;
        var n_verts = n + 2;
    ...
    class SphereVertexCache {
        ...
        getVerticesForRadius(radius: number, sphereQuality: any) {
            ...
            if (radiusCache.has(radius)) return radiusCache.get(radius);
    ...
    export function drawSphere(
        ...
        var vobj = sphereVertexCache.getVerticesForRadius(
            radius,
            sphereQuality
        );
        ...
        for (let y = 0; y < h; y++) {
            ...
            for (let x = 0; x < w; x++) {
                let faceoffset = geoGroup.faceidx,
                    lineoffset = geoGroup.lineidx;

                let v1 = verticesRows[y][x + 1] + start,
                    v1offset = v1 * 3;
                let v2 = verticesRows[y][x] + start,
                    v2offset = v2 * 3;
                let v3 = verticesRows[y + 1][x] + start,
                    v3offset = v3 * 3;
                let v4 = verticesRows[y + 1][x + 1] + start,
                    v4offset = v4 * 3;
