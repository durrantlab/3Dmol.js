...
export function autoload(viewer?: any, callback?: (arg0: any) => void) {
    ...
    if (document.querySelector(".viewer_3Dmoljs") != null) autoinit = true;

    if (autoinit) {
        processing_autoinit = true;
        viewer = viewer != undefined ? viewer : null;
        var nviewers = 0;
        const elements = document.querySelectorAll(".viewer_3Dmoljs");
        
        elements.forEach((viewerdiv) => {
            var datauri = [];
            var datatypes = [];
            var uri = "";
            ...
            if (viewerdiv.dataset.pdb) {
                ...
                datauri.push(
                    "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/" +
                        viewerdiv.dataset.cid +
                        "/SDF?record_type=3d"
                );
            } else if (viewerdiv.dataset.href || viewerdiv.dataset.url) {
                if (viewerdiv.dataset.href) uri = viewerdiv.dataset.href;
                else uri = viewerdiv.dataset.url;
                datauri.push(uri);
                ...
                viewerdiv.dataset[datatypes[datatypes.length - 1]] =
                    molName;
            }
            ...
            var bgalpha: string | number =
                viewerdiv.dataset.backgroundalpha;
            bgalpha = bgalpha == undefined ? 1.0 : parseFloat(bgalpha as string);
            var style = { line: {} };
            if (viewerdiv.dataset.style)
                style = specStringToObject(viewerdiv.dataset.style);
            var select = {};
            if (viewerdiv.dataset.select)
                select = specStringToObject(viewerdiv.dataset.select);
            var selectstylelist = [];
            var surfaces = [];
            var labels = [];
            var zoomto = {};
            ...
            var applyStyles = function (glviewer: GLViewer) {
                ...
                for (i = 0; i < surfaces.length; i++) {
                    ...
                    if (UI) {
                        viewer
                            .addSurface(SurfaceType.VDW, sty, sel, sel)
                            .then((surfid: any) => {
            ...
            try {
                var config: any =
                    specStringToObject(viewerdiv.dataset.config) || {};
                if (config.backgroundColor === undefined)
                    config.backgroundColor = bgcolor;
                if (config.backgroundAlpha === undefined)
                    config.backgroundAlpha = bgalpha;
                if (glviewer == null) {
                    glviewer = viewers[viewerdiv.id || nviewers++] =
                        createViewer(viewerdiv, config);
                } else {
            ...
            if (datauri.length != 0) {
                ...
                var process = function (moldata: any) {
                    ...
                    var type =
                        viewerdiv.dataset.type ||
                        viewerdiv.dataset.datatype ||
                        datatypes[i];
                    ...
                    if (i < datauri.length) {
                        ...
                        if (viewerdiv.dataset.callback) {
                            ...
                            /*jshint -W061 */ var runres = eval(
                                viewerdiv.dataset.callback
                            );
