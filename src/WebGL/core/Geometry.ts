...
export class GeometryGroup {
    ...
    setColors(
        setcolor: (x: number, y: number, z: number) => Color | number
    ): void {
  ...
  as rectangles rather than two triangles) */
    ...
    vrml(indent: string, material?: Material) {
        ...
        if (material?.transparent) {
            ret +=
                indent + "   transparency " + (1.0 - material.opacity) + "\n";
        }
        ret += indent + "  }\n"; //material
        ret += indent + " }\n"; //appearance
    ...
    truncateArrayBuffers(mesh = true, reallocatemem = false) {
        ...
        if (mesh) {
            this.normalArray =
                normalArr?.subarray(0, this.vertices * 3) || null;
...
export class Geometry extends EventDispatcher {
    ...
    updateGeoGroup(addVertices = 0): GeometryGroup {
        ...
        if (
            !retGroup ||
            retGroup.vertices + addVertices >
                (retGroup?.vertexArray?.length || 0) / 3
        )
    ...
    setUpWireframe(
        ...lineIndexArgs: Parameters<GeometryGroup["setLineIndices"]>
    ) {
        let len = this.geometryGroups.length;
        for (var g = 0; g < len; g++) {
            var geoGroup = this.geometryGroups[g];
